...
import { Utensils, Egg, Scissors, Clock } from "lucide-react";
...
export default function AnimalCard({ animal, onFeed, onCollect, onSlaughter }: AnimalCardProps) {
  ...
  const hoursElapsed = isEggReady ? (now - animal.lastCollectedAt) / 3600000 : 0;
  const cappedHours = Math.min(24, Math.max(0, hoursElapsed));
  const accumulatedEggs = isEggReady
    ? Math.floor(cappedHours / type.productionIntervalHours) * type.eggYield
    : 0;

  // Timer until next egg
  ...
  return (
    ...
    >
      ...
      {/* Egg info for grown egg-type animals */}
      {isEggReady && (
        <div className="mb-3 rounded-xl p-2.5 border border-accent/30" style={{
          background: 'hsl(45 70% 88% / 0.5)'
        }}>
      ...
      {/* Actions — single primary "Boqish" button, secondary collect/slaughter */}
