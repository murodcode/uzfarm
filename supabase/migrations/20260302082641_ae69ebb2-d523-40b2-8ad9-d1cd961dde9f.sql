...
BEGIN
  ...
  ELSIF p_column = 'cash' THEN
    SELECT cash INTO user_val FROM profiles WHERE id = p_user_id;
    SELECT COUNT(*) + 1 INTO rank_val FROM profiles WHERE cash > COALESCE(user_val, 0);
  ELSE
